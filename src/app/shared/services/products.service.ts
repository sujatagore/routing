...
export class ProductsService {
  ...
  productsArr : Array<Iproducts> = [
    // {
    //     pname : 'OnePlus Nord CE 4 5G',
    //     pid : '145',
    //     pstatus : "In Progress",
    //     canReturn : 1
    // },
    // {
    //   pname : 'OPPO F27 Pro Plus',
    //   pid : '135',
    //   pstatus : 'Dispatched',
    //   canReturn : 0
    // },
    // {
    //   pname : 'Motorola Edge 50 Fusion',
    //   pid : '156',
    //   pstatus : 'Delivered',
    //   canReturn : 1
    // }
  ...
  getProductDetails(id : string): Iproducts{
      return this.productsArr.find(pro => pro.pid === id) as Iproducts
  }
  ...
  updateProduct(uptProdObj : Iproducts){
      let getIndex  = this.productsArr.findIndex(pro => pro.pid === uptProdObj.pid);
      let updatedObj = this.productsArr[getIndex]
      this.productsArr[getIndex] = uptProdObj;
      this._router.navigate(['/products']);
      this._snackbar.openSnackBar(`The Product ${updatedObj.pname} is updated to ${uptProdObj.pname} Successfully!!!`)
  }
